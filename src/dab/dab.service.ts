...
export class DabService {
  ...
  async login(cardNumber: string, pin: string) {
    const card = await this.creditCardService.findOneByCardNumberAndPin(
      cardNumber,
      pin,
    );
    if (!card) {
  ...
  async getLastTransactions(accountId: number) {
    ...
    const transactions =
      await this.transactionService.findLastTransactions(accountId);
  ...
  async transfer(fromAccountId: number, toAccountId: number, amount: number) {
    ...
    if (!fromAccount || !toAccount) {
      throw new BadRequestException(
        "L'un des comptes spécifiés est introuvable.",
      );
    }

    if (
      fromAccount.userId !== this.currentUser.id ||
      toAccount.userId !== this.currentUser.id
    ) {
      throw new UnauthorizedException(
        "Les virements ne peuvent être effectués qu'entre les comptes de l'utilisateur.",
      );
    }

    if (fromAccount.balance < amount) {
      throw new BadRequestException(
        "Solde insuffisant pour le compte d'origine.",
      );
    }
  ...
  async depositCheck(
    ...
    if (account.userId !== this.currentUser.id) {
      throw new UnauthorizedException(
        'Vous ne pouvez déposer un chèque que sur vos propres comptes.',
      );
    }
    ...
    return {
      message: 'Le dépôt de chèque sera crédité après le délai spécifié.',
    };
  }
